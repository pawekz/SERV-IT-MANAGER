...
const BeforePicturesGallery = ({ photos = [] }) => {
  ...
  return (
    <div className="mb-6 max-w-3xl mx-auto">
      <h3 className="text-md font-semibold mb-2">Before Pictures</h3>
      <div className="flex flex-wrap gap-2">
        {photos.map((url, idx) => (
          <div key={idx} className="cursor-pointer" onClick={() => openModal(idx)}>
            <TicketImage path={url} alt={`Repair photo ${idx + 1}`} className="w-24 h-24 object-cover rounded border" />
          </div>
        ))}
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-90 flex items-center justify-center z-50" onClick={closeModal}>
          <div className="relative max-w-[90vw] max-h-[90vh] bg-black rounded-lg" onClick={e => e.stopPropagation()}>
            ...
            <div className="relative flex items-center justify-center">
              <TicketImage path={photos[currentIdx]} alt={`Repair photo ${currentIdx + 1}`} className="max-w-[90vw] max-h-[85vh] object-contain" />
              {photos.length > 1 && (
                <>
